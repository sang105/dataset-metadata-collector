...
export function ObservationFormLayout() {
    ...
    return (
    <SectionShell icon="⬡" label="Observations" color="#6366F1">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <span style={S.subheading}>Observations ({obs.length})</span>
        <button onClick={() => {            
            dispatch({ type: "ADD_OBSERVATION" })
        } 
        } style={{ background: "#6366F111", border: "1px solid #6366F133", color: "#6366F1", borderRadius: 7, padding: "5px 14px", fontSize: 12, cursor: "pointer", fontFamily: "'DM Mono', monospace" }}>
            + Add Observation
        </button>
      </div>
      ...
      {obs.map((o, i) => {
        ...
        return (
          <div key={o.id} style={{ border: "1px solid #1E2D45", borderRadius: 10, padding: 16, marginBottom: 14, background: "#070F1C" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
              <span style={{ fontSize: 11, color: "#334155", fontFamily: "'DM Mono', monospace" }}>Observation #{i + 1}</span>
              <button onClick={() => dispatch({ type: "REMOVE_OBSERVATION", id: o.id })} style={S.removeBtn}>×</button>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                <label style={S.label}>Observed Node</label>
                <select value={o.observedNode} onChange={e => upd("observedNode", e.target.value)} style={{ ...S.input, cursor: "pointer" }}>
              ...
              <div>
                ...
                <select value={o.measuredProperty} onChange={e => upd("measuredProperty", e.target.value)} style={{ ...S.input, cursor: "pointer" }}>
                  <option value="">Select…</option>
              ...
              <div>
                ...
                <input type="number" value={o.measuredValue} onChange={e => upd("measuredValue", e.target.value)} placeholder="1000000" style={S.input} />
              </div>
              <div>
                <label style={S.label}>Observation Date</label>
                <input type="date" value={o.observationDate} onChange={e => upd("observationDate", e.target.value)} style={S.input} />
              </div>
            </div>
